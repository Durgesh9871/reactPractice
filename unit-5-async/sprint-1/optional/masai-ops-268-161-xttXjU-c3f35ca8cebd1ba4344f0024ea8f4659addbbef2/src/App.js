    ...
    <div className="App">
      <div>
        ...
        <div className="form-wrapper" data-testid="form-wrapper">
          <form data-testid="form-element" onSubmit={handleChange}>
            ...
            >
              ...
              <div>
                ...
                <input type={"checkbox"} name="maritalStatus" value={state.maritalStatus} checked={state.maritalStatus} onChange={(e)=> dispatch({type:"checkbox" ,payload:true})} />
                <label>Married</label>
              </div>
            </div>
            <div>
        ...
        {/* map through the submittedData and render UserRow component to display the data in tabular format ELSE print "no users found"  in there is no user data */}
         ...
         {submittedData.length == 0 && (<h2 data-testid="no-user-container">no users found</h2>)} 
          { submittedData.length > 0 && 
            <table>
              ...
              <tbody>
                {submittedData.map((el,id)=>(
                  <UserRow key={id} name={el.name} gender={el.gender} role={el.role} maritalStatus={el.maritalStatus? "married" :"unmarried"} id ={id+1}  />
                ))}
              </tbody>
            </table>
          }
